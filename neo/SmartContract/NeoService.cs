...
using Neo.IO;
using Neo.SmartContract.CrossChain;
...
namespace Neo.SmartContract
{
    public class NeoService : StandardService
    {
            ...
            : base(trigger, snapshot)
        {
            ...
            Register("Neo.TransactionState.Set", Set_TransactionState, 1);
            Register("Neo.TransactionState.Get", Get_TransactionState, 1);
            #endregion

            #region CrossChainVerification
            Register("Neo.CrossMerkleTree.MerkleProve", MerkleProve, 0);
            #endregion

            #region CrossChainRegister
            Register("Neo.CrossChain.SyncBlockHeader", CrossChain_SyncBlockHeader, 0);
            Register("Neo.CrossChain.CreateTransaction", CrossChain_CreateTransaction, 0);
            Register("Neo.CrossChain.ProcessCrossChainTransaction", ProcessCrossChainTransaction, 0);
            Register("Neo.CrossChain.ToHexString", CrossChain_ToHexString, 0);
            Register("Neo.CrossChain.IntToString", CrossChain_ConvertIntToString, 0);
            Register("Neo.CrossChain.GetHeaderByHeight", CrossChain_GetHeaderByHeight, 0);
            Register("Neo.CrossChain.GetCurrentHeight", CrossChain_GetCurrentHeight, 0);
            Register("Neo.CrossChain.GetBlockHeaderHash", CrossChain_GetBlockHeaderHash, 0);
            Register("Neo.CrossChain.DeserializeParameters", DeserializeParameters, 0);
            #endregion
        ...
        /// <returns></returns>
        private bool CrossChain_CreateTransaction(ExecutionEngine engine)
        {
            var toChainID = engine.CurrentContext.EvaluationStack.Pop().GetBigInteger();
            var toChainAddress = new UInt160(engine.CurrentContext.EvaluationStack.Pop().GetByteArray());
            var functionName = engine.CurrentContext.EvaluationStack.Pop().GetString();
            var args = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();

            var txID = ((Transaction)engine.ScriptContainer)?.Hash ?? ConstResource.DEFAULT_TX_HASH;
            var contract = engine.CurrentContext.ScriptHash;//当前合约
            var callerContract = engine.CallingContext.ScriptHash;//发起交易合约
            var para = new CreateCrossChainTxParam()
            {
                TxID = txID,
                FromChainID = ConstResource.NEO_CHAIN_ID,
                FromContractAddress = callerContract,
                ToChainID = (ulong)toChainID,
                FunctionName = functionName,
                Args = args,
            };
            CrossChain_Log(engine, $"正在创建跨链交易：{toChainID},{toChainAddress},{functionName},{args.ToHexString()}");
            var storageContext = new StorageContext()
            {
                ScriptHash = new UInt160(contract),
            };
            var requestID = GetRequestID(storageContext, para.ToChainID);
            var newID = requestID + 1;
            //var merkleValue = new MerkleValue()
            //{
            //    RequestID = newID,
            //    Para = para,
            //};

            var merkleData = para.ToArray();

            PutRequest(storageContext, newID, para.ToChainID, merkleData);

            //Add merkleData into executionEngine
            //native.ContextRef.PutMerkleVal(sink.Bytes())
            if (engine is ApplicationEngine ae)
            {
                ae.TransactionState = merkleData;
            }

            PutRequestID(storageContext, newID, para.ToChainID);


            //生成通知
            CrossChain_Notify(engine, new VMArray()
            {
                new ByteArray("create_cross_tx_success".ToUTF8Bytes()),
                new ByteArray(para.ToChainID.ToBigIntegerBytes()),
                new ByteArray(newID.ToBigIntegerBytes()),
                new ByteArray(Snapshot.Height.ToBigIntegerBytes()),
                new ByteArray(merkleData),
                new VMString($"生成跨链交易成功:toChain[{para.ToChainID}],requestID[{newID}],height[{Snapshot.Height}]"),
            });

            engine.CurrentContext.EvaluationStack.Push(newID);
            return true;
        }

        private bool MerkleProve(ExecutionEngine engine)
        {
            var proof = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
            var cross_states_root = new UInt256(engine.CurrentContext.EvaluationStack.Pop().GetByteArray());
            var result = CrossMerkleTree.MerkleProve(proof, cross_states_root);
        ...
        /// <returns></returns>
        private ulong GetRequestID(StorageContext context, ulong chainID)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();
            var value = CrossChain_Get(context,
                "REQUEST_ID".ToUTF8Bytes().ConcatArray(chainIDBytes));
            if (value != null)
            {
        ...
        /// </summary>
        /// <param name="context"></param>
        /// <param name="chainID">目标链ID</param>
        /// <param name="requestID"></param>
        /// <returns></returns>
        private bool PutRequestID(StorageContext context, ulong requestID, ulong chainID)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();
            var requestIDBytes = requestID.ToBigIntegerBytes();
            return CrossChain_PutEx(context,
                "REQUEST_ID".ToUTF8Bytes().ConcatArray(chainIDBytes),
                requestIDBytes);
        }
        ...
        /// <param name="requestID"></param>
        /// <param name="chainID"></param>
        /// <param name="merkleData">跨链请求参数信息</param>
        /// <returns></returns>
        private bool PutRequest(StorageContext context, ulong requestID, ulong chainID, byte[] merkleData)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();
            var requestIDBytes = requestID.ToBigIntegerBytes();
            return CrossChain_PutEx(context,
                "REQUEST".ToUTF8Bytes().ConcatArray(chainIDBytes, requestIDBytes),
                merkleData);
        }
        ...
        /// <param name="engine"></param>
        /// <returns></returns>
        private bool CrossChain_SyncBlockHeader(ExecutionEngine engine)
        {
            var rawData = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
            var para = rawData.AsSerializable<SyncBlockHeaderParam>();
            if (para.Headers == null || para.Headers.Count == 0) return true;
            foreach (var headerBytes in para.Headers)
            {
                var header = headerBytes.AsSerializable<RelayChainHeader>();
                var contract = engine.CurrentContext.ScriptHash;//HeaderSync合约地址（当前执行的合约地址）
                var storageContext = new StorageContext
                {
                    ScriptHash = new UInt160(contract),
                    IsReadOnly = false
                };
                var chainIDBytes = header.ChainID.ToBigIntegerBytes();
                var heightBytes = header.Height.ToBigIntegerBytes();
                var blockHash = header.Hash();

                //校验去重
                var oldHeaderBytes = GetHeaderByHeight(storageContext, header.ChainID, header.Height);
                if (oldHeaderBytes != null)
                {
                    try
                    {
                        var oldHeader = oldHeaderBytes.AsSerializable<RelayChainHeader>();
                        CrossChain_Log(engine, $"Chain[{header.ChainID}]<{header.Height}>已有区块头，将被忽略:{oldHeaderBytes.ToHexString()}");
                        continue;
                    }
                    catch (Exception e)
                    {
                        CrossChain_Log(engine, $"Chain[{header.ChainID}]<{header.Height}>已存入了不合法的区块头，将被覆盖:{oldHeaderBytes.ToHexString()}");
                    }
                }
                ...
                //存头部完整数据：blockhash=>rawdata
                CrossChain_PutEx(storageContext,
                    "BLOCK_HEADER".ToUTF8Bytes().ConcatArray(chainIDBytes, blockHash.ToArray()),
                    headerBytes);

                //存高度=>blockhash
                CrossChain_PutEx(storageContext,
                    "HEADER_INDEX".ToUTF8Bytes().ConcatArray(chainIDBytes, heightBytes),
                    blockHash.ToArray());

                //存最新高度:ChainId=>高度
                CrossChain_PutEx(storageContext,
                    "CURRENT_HEIGHT".ToUTF8Bytes().ConcatArray(chainIDBytes),
                    heightBytes);

                //生成通知
                CrossChain_Notify(engine, new VMArray()
                {
                    new ByteArray("sync_header_success".ToUTF8Bytes()),
                    new Integer((BigInteger)header.ChainID),
                    new VMString($"已经同步RelayChain[{header.ChainID}]区块头:{header.Height}")
                });

                //err = UpdateConsensusPeer(native, header, params.Address)
                var jsonString = header.ConsensusPayload.ToUTF8String();
                var blkInfo = JsonConvert.DeserializeObject<VbftBlockInfo>(jsonString);
                if (blkInfo.NewChainConfig != null)
                {
                    if (header.ChainID != 0)
                    {
                    ...
                    var consensusPeers = new ConsensusPeers()
                    {
                        ChainID = header.ChainID,
                        Height = header.Height,
                        PeerMap = new Dictionary<string, Peer>()
                    };
                    foreach (var peerConfig in blkInfo.NewChainConfig.Peers)
                    {
                        consensusPeers.PeerMap[peerConfig.ID] = new Peer()
                        {
                            Index = peerConfig.Index,
                            PeerPubkey = peerConfig.ID,
                        };
                    }

                    //存共识公钥
                    PutConsensusPeers(storageContext, consensusPeers);

                    //生成通知
                    CrossChain_Notify(engine, new VMArray()
                    {
                        new ByteArray("sync_consensus_success".ToUTF8Bytes()),
                        new Integer((BigInteger)header.ChainID),
                        new VMString($"已经同步RelayChain[{header.ChainID}]共识:{header.Height}"),
                    });
                }
            }
        ...
        /// <param name="engine"></param>
        /// <returns></returns>
        private bool CrossChain_GetHeaderByHeight(ExecutionEngine engine)
        {
            var chainID = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
            var height = engine.CurrentContext.EvaluationStack.Pop().GetBigInteger().ToByteArray();
            var contract = engine.CurrentContext.ScriptHash;
            var storageContext = new StorageContext
            {
                ScriptHash = new UInt160(contract),
                IsReadOnly = false
            ...
            var header = GetHeaderByHeight(storageContext, chainID, height);
            engine.CurrentContext.EvaluationStack.Push(header ?? new byte[0]);
            return true;
        }

        /// <summary>
        /// 根据chainID查询已经保存的区块头最新高度
        /// </summary>
        /// <param name="engine"></param>
        /// <returns></returns>
        private bool CrossChain_GetCurrentHeight(ExecutionEngine engine)
        {
            var chainIDBytes = engine.CurrentContext.EvaluationStack.Pop().GetBigInteger().ToByteArray();
            var contract = engine.CurrentContext.ScriptHash;//HeaderSync合约地址（当前执行的合约地址）
            var storageContext = new StorageContext
            {
                ScriptHash = new UInt160(contract),
            ...
            //存最新高度:ChainId=>高度
            var value = CrossChain_Get(storageContext,
                "CURRENT_HEIGHT".ToUTF8Bytes().ConcatArray(chainIDBytes));
            engine.CurrentContext.EvaluationStack.Push(value ?? new byte[0]);
            return true;
        }


        /// <summary>
        /// 根据chainID和height查询区块头hash
        /// </summary>
        /// <param name="engine"></param>
        /// <returns></returns>
        private bool CrossChain_GetBlockHeaderHash(ExecutionEngine engine)
        {
            var chainIDBytes = engine.CurrentContext.EvaluationStack.Pop().GetBigInteger().ToByteArray();
            var heightBytes = engine.CurrentContext.EvaluationStack.Pop().GetBigInteger().ToByteArray();
            var contract = engine.CurrentContext.ScriptHash;//HeaderSync合约地址（当前执行的合约地址）
            var storageContext = new StorageContext
            {
                ScriptHash = new UInt160(contract),
            ...
            //存最新高度:ChainId=>高度
            var value = CrossChain_Get(storageContext,
                "HEADER_INDEX".ToUTF8Bytes().ConcatArray(chainIDBytes).ConcatArray(heightBytes));
            engine.CurrentContext.EvaluationStack.Push(value ?? new byte[0]);
            return true;
        }
        ...
        /// <param name="consensusPeers"></param>
        private void PutConsensusPeers(StorageContext storageContext, ConsensusPeers consensusPeers)
        {
            var data = consensusPeers.ToArray();
            var chainIDBytes = consensusPeers.ChainID.ToBigIntegerBytes();
            var heightBytes = consensusPeers.Height.ToBigIntegerBytes();

            //存共识节点:高度=>共识信息
            CrossChain_PutEx(storageContext,
                "CONSENSUS_PEER".ToUTF8Bytes().ConcatArray(chainIDBytes, heightBytes), data);

            var keyHeights = GetKeyHeights(storageContext, consensusPeers.ChainID);
            keyHeights.HeightList.Add(consensusPeers.Height);

            PutKeyHeights(storageContext, consensusPeers.ChainID, keyHeights);
        ...
        /// </summary>
        /// <param name="storageContext"></param>
        private ConsensusPeers GetConsensusPeers(StorageContext storageContext, ulong chainID, ulong height)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();
            var heightBytes = height.ToBigIntegerBytes();
            //取共识节点信息:高度=>共识信息
            var data = CrossChain_Get(storageContext, "CONSENSUS_PEER".ToUTF8Bytes().ConcatArray(chainIDBytes, heightBytes));
            return data.AsSerializable<ConsensusPeers>();
        }

        private bool VerifyHeader(StorageContext storageContext, RelayChainHeader header)
        {
            var keyHeight = FindKeyHeight(storageContext, header.ChainID, header.Height);

            ////check epoch
            //chainIBytes, err:= utils.GetUint64Bytes(header.ChainID)
            ...
            var consensusPeer = GetConsensusPeers(storageContext, header.ChainID, keyHeight);
            if (consensusPeer == null)
            {
                throw new Exception($"找不到共识信息:Chain[{header.ChainID}]<{header.Height}>-{keyHeight}");
            }
            var publickeys = new List<byte[]>();
            foreach (var bookkeeper in header.Bookkeepers)
            {
#warning 公钥转换可能有问题
                var pubkey = bookkeeper.ToHexString();
                var presentKey = consensusPeer.PeerMap.ContainsKey(pubkey) ? consensusPeer.PeerMap[pubkey] : null;
                if (presentKey == null)
                {
                    throw new Exception($"无效的pubkey:{pubkey}");
                }
                publickeys.Add(bookkeeper);
            }

            var hash = header.Hash().ToArray();

            return ConstResource.VerifyMultiSignature(hash, publickeys, publickeys.Count, header.SigData);
        }



        /// <summary>
        /// 查询某chain共识节点变更的所有区块高度列表
        /// </summary>
        /// <param name="storageContext"></param>
        /// <param name="chainID"></param>
        /// <returns></returns>
        private KeyHeight GetKeyHeights(StorageContext storageContext, ulong chainID)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();

            var value = CrossChain_Get(storageContext, "KEY_HEIGHTS".ToUTF8Bytes().ConcatArray(chainIDBytes));
            if (value == null)
            {
                return new KeyHeight();
            }
            var keyHeights = value.AsSerializable<KeyHeight>();
            return keyHeights;
        }

        /// <summary>
        /// 更新某chain共识节点变更的所有区块高度列表
        /// </summary>
        /// <param name="storageContext"></param>
        /// <param name="chainID"></param>
        /// <param name="keyHeights"></param>
        /// <returns></returns>
        private bool PutKeyHeights(StorageContext storageContext, ulong chainID, KeyHeight keyHeights)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();
            var data = keyHeights.ToArray();

            return CrossChain_PutEx(storageContext,
                "KEY_HEIGHTS".ToUTF8Bytes().ConcatArray(chainIDBytes),
                data);
        }
        ...
        /// <returns></returns>
        private uint FindKeyHeight(StorageContext storageContext, ulong chainID, uint height)
        {
            var keyHeights = GetKeyHeights(storageContext, chainID);
            return keyHeights.HeightList.OrderByDescending(h => h).FirstOrDefault(h => height > h);
        }
        ...
        /// <param name="height"></param>
        /// <returns></returns>
        private byte[] GetHeaderByHeight(StorageContext storageContext, ulong chainID, ulong height)
        {
            var chainIDBytes = chainID.ToBigIntegerBytes();
            var heightBytes = height.ToBigIntegerBytes();
            return GetHeaderByHeight(storageContext, chainIDBytes, heightBytes);
        }
        ...
        /// </summary>
        /// <param name="storageContext"></param>
        /// <param name="chainIDBytes"></param>
        /// <param name="heightBytes"></param>
        /// <returns></returns>
        private byte[] GetHeaderByHeight(StorageContext storageContext, byte[] chainIDBytes, byte[] heightBytes)
        {
            //取头部hash
            var txHash = CrossChain_Get(storageContext, "HEADER_INDEX".ToUTF8Bytes().ConcatArray(chainIDBytes, heightBytes));
            if (txHash == null)
            {
                return null;
            }
            //取头部完整数据：blockhash=>rawdata
            return CrossChain_Get(storageContext, "BLOCK_HEADER".ToUTF8Bytes().ConcatArray(chainIDBytes, txHash));
        }
        ...
        private void CrossChain_Log(ExecutionEngine engine, string message)
        {
        ...
        /// <param name="engine"></param>
        /// <param name="message"></param>
        private void CrossChain_Notify(ExecutionEngine engine, StackItem message)
        {
            engine.CurrentContext.EvaluationStack.Push(message);
            Runtime_Notify(engine);
        }


        private byte[] CrossChain_Get(StorageContext context, byte[] key)
        {
            if (!CheckStorageContext(context))
            {
                throw new Exception("无效的存储区");
            }
        ...
        private bool CrossChain_PutEx(StorageContext context, byte[] key, byte[] value, StorageFlags flags = StorageFlags.None)
        {
        ...
        /// <param name="engine"></param>
        /// <returns></returns>
        private bool CrossChain_ToHexString(ExecutionEngine engine)
        {
            var data = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
            var text = data.ToHexString();
            engine.CurrentContext.EvaluationStack.Push(text);
            return true;
        }
        ...
        /// <param name="engine"></param>
        /// <returns></returns>
        private bool CrossChain_ConvertIntToString(ExecutionEngine engine)
        {
            var data = engine.CurrentContext.EvaluationStack.Pop().GetBigInteger();
            engine.CurrentContext.EvaluationStack.Push(data.ToString());
            return true;
        }
        ...
        private bool Set_TransactionState(ExecutionEngine engine)
        {
            if (Trigger != TriggerType.Application && Trigger != TriggerType.ApplicationR) return false;
            if (engine is ApplicationEngine ae)
            {
                ae.TransactionState = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
            }
            return true;
        }
        private bool Get_TransactionState(ExecutionEngine engine)
        {
            if (engine is ApplicationEngine ae)
            {
                ae.TransactionState = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
                if (ae.TransactionState == null)
                {
                    return false;
                }
                else
                {
                    engine.CurrentContext.EvaluationStack.Push(ae.TransactionState);
                }
            }
            return true;
        }

        private bool DeserializeParameters(ExecutionEngine engine)
        {
            byte[] ToContractScriptHash;
            string Method;
            byte[] t;
            ulong Value;
            try
            {
                var path = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
                var root = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
                //byte[] source = CrossMerkleTree.MerkleProve(path, new UInt256(root));
                //if (source == null) return false;
                //using (MemoryStream memoryStream = new MemoryStream(source))
                using (MemoryStream memoryStream = new MemoryStream(path))
                using (BinaryReader binaryReader = new BinaryReader(memoryStream))
                {
                    binaryReader.ReadUInt64();
                    byte[] TxHash1 = binaryReader.ReadBytes(32); // TxHash1
                    var temp = binaryReader.ReadVarString();
                    ToContractScriptHash = temp.HexToBytes(); // ToContractScriptHash
                    byte[] TxHash2 = binaryReader.ReadVarBytes(); // TxHash2
                    UInt64 FromChainID = binaryReader.ReadUInt64(); //FromChainID
                    binaryReader.ReadVarBytes(); //FromContractAddress
                    binaryReader.ReadUInt64(); //ToChainID
                    Method = binaryReader.ReadVarString(); //Method
                    t = binaryReader.ReadVarBytes();
                    CrossChain_Notify(engine, new VMArray()
                    {
                        new ByteArray("verifyToNeoProof".ToUTF8Bytes()),
                        new ByteArray(TxHash2),
                        new ByteArray(TxHash1),
                        new BigInteger(FromChainID),
                        new ByteArray(ToContractScriptHash)
                    }) ;
                }
                VMArray result = new VMArray();
                result.Add(ToContractScriptHash);
                result.Add(Method);
                result.Add(t);
                engine.CurrentContext.EvaluationStack.Push(result);
                return true;
        ...
        private bool ProcessCrossChainTransaction(ExecutionEngine engine)
        {
            var path = engine.CurrentContext.EvaluationStack.Pop().GetByteArray();
            using (MemoryStream memoryStream1 = new MemoryStream(path))
            using (BinaryReader binaryReader1 = new BinaryReader(memoryStream1))
            {
                var arg = (Map)DeserializeStackItem(binaryReader1, engine); // address.
                engine.CurrentContext.EvaluationStack.Push(arg);
            }
            return true;
        }
    }
