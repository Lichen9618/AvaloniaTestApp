    ...
    public sealed class RpcServer : IDisposable
    {
        ...
        private JObject Process(string method, JArray _params)
        {
            switch (method)
            {
                ...
                case "getcrossmerklevalue":
                    {
                        UInt256 tx_hash = UInt256.Parse(_params[0].AsString());
                        return GetCrossMerkleValue(tx_hash);
                    }
                case "getcrossmerkletree":
                    {
                        uint height = uint.Parse(_params[0].AsString());
                        return GetCrossMerkleTree(height);
                    }
                case "getcrossproof":
                    {
                        uint height = uint.Parse(_params[0].AsString());
                        UInt256 tx_hash = UInt256.Parse(_params[1].AsString());
                        return GetCrossProof(height, tx_hash);
                    }
                case "getcrossprove":
                    {
                        JObject path = _params[0];
                        UInt256 cross_states_root = UInt256.Parse(_params[1].AsString());
                        return GetCrossProve(path, cross_states_root);
                    }
                case "getblocksysfee":
                    {
                        uint height = uint.Parse(_params[0].AsString());
        ...
        private JObject GetCrossMerkleValue(UInt256 tx_hash)
        {
            StorageItem merkle_value = Blockchain.Singleton.Store.GetCrossMerkleValue().TryGet(tx_hash);
            return merkle_value.Value?.ToHexString() ?? throw new RpcException(-100, "Unknown tx_hash");
        }

        private JObject GetCrossMerkleTree(uint height)
        {
            CrossMerkleValueList merkle_value_list = Blockchain.Singleton.Store.GetCrossMerkleTree().TryGet(height);
            return merkle_value_list?.merkle_value[0].ToHexString()?? throw new RpcException(-100, "Unknown height");
        }

        private JObject GetCrossProof(uint height, UInt256 tx_hash)
        {
            CrossMerkleValueList merkle_value_list = Blockchain.Singleton.Store.GetCrossMerkleTree().TryGet(height);
            StorageItem item = Blockchain.Singleton.Store.GetCrossMerkleValue().TryGet(tx_hash);
            if (merkle_value_list == null || item == null)
                throw new RpcException(-100, "no merkle_value");
            UInt256[] hashes = new UInt256[merkle_value_list.merkle_value.Count];
            for (int i = 0; i < merkle_value_list.merkle_value.Count; i++)
            {
                UInt256 hash = new UInt256(CrossMerkleTree.HashLeaf(merkle_value_list.merkle_value[i]));
                hashes[i] = hash;
            }
            byte[] proof = CrossMerkleTree.GetMerkleProof(item.Value, hashes);
            return proof?.ToHexString() ?? throw new RpcException(-100, "no proof");
        }

        private JObject GetCrossProve(JObject path, UInt256 cross_states_root)
        {
            byte[] proof = Helper.HexToBytes(path.AsString());
            byte[] result = CrossMerkleTree.MerkleProve(proof, cross_states_root);
            return result?.ToHexString() ?? throw new RpcException(-100, "prove error");
        }
